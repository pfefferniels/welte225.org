"""Passage by passage: does the performance follow the base of the scan, or the base with one cluster of readings toggled?"""
import argparse
import json
import math
...
def span_class(cluster: dict) -> str:
    """'lone punch' where a command has no partner of its function and register in the cluster; otherwise whether
    the widest distance from an On to the next Off is shorter than 30 mm."""
    lane_of = lambda s: (s['type'].removesuffix('On').removesuffix('Off'), s['scope'])
    lanes = [[s for s in cluster['symbols'] if lane_of(s) == lane] for lane in {lane_of(s) for s in cluster['symbols']}]

    def widest(commands: list[dict]) -> float | None:
        ons = [s['from'] for s in commands if s['type'].endswith('On')]
        offs = [s['from'] for s in commands if s['type'].endswith('Off')]
        return max(min((off - on for off in offs if off > on), default=math.inf) for on in ons) if ons and offs else None

    spans = [widest(commands) for commands in lanes]
    if any(span is None for span in spans):
        return 'lone punch'
    return 'short pairs' if max(spans) < SHORT_PAIR_MM else 'long span'


def circular_llr_null(residual: np.ndarray, positions: np.ndarray, d: np.ndarray, sigma2: float) -> np.ndarray:
    """LLR of the toggle under the base, with the base's residuals shifted around the piece so their autocorrelation is kept."""
...
def main():
    ...
    parser.add_argument('--scan', type=Path, default=SCRATCH / 'emu' / 'scan_A.json')
