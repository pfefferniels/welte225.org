...
def emulated_notes(version) -> list[Note]:
    ...
    return [Note(n['id'], n['pitch'], n['at'], offsets.get(n['id'], n['at'] + 0.3), n['velocity']) for n in version['notes']]
...
def spline_time_map(reference: list[Note], matches: dict[str, Note]):
    ...
    spline = UnivariateSpline(x, y, k=3, s=len(x) * 0.03 ** 2)
    return spline
...
def main():
    ...
    parser.add_argument('--reference', default='B')
    ...
    velocities = {s: {n['id']: n['velocity'] for n in versions[s]['notes']} for s in SIGLA}
    moved_note = {'symbol_c365caf3-577d-4cb6-b5dd-ec9221af00d0': 'symbol_bc5958aa-5fee-4daf-8457-9d96b98208a4'}
    rows = []
    for note in reference:
        rec = matches.get(note.id)
        row = {'id': note.id, 'pitch': note.pitch, 'emu_t': note.onset, 'emu_off': note.offset,
               'rec_t': rec.onset if rec else None, 'rec_off': rec.offset if rec else None,
               'kong_velocity': rec.velocity if rec else None}
        row.update({f'V_{s}': velocities[s].get(note.id, velocities[s].get(moved_note.get(note.id, ''))) for s in SIGLA})
        rows.append(row)
